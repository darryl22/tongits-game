...
function Signup() {
    ...
    function handleSubmit(e) {
        ...
        axios.post("http://localhost:3002/signup",{data: formData}, {withCredentials: "true"})
        .then(res => {
            console.log(res.data)
            if (res.data.error) {
                setError(res.data.error)
            } else {
                setRedirectTo([true, "/login"])
            }
        })
        .catch(err => {
            console.log(err)
    ...
    return (
        <div>
            ...
            <form action="" className="auth-form" onSubmit={handleSubmit}>
                ...
                <input type="text" name="confirmpassword" value={formData.confirmpass} className="auth-input" onChange={hamdleChange}/>
